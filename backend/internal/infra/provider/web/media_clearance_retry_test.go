...
func TestIsClearanceRefreshableMediaError(t *testing.T) {
	tests := []struct {
		...
		{name: "empty forbidden response", code: http.StatusForbidden, want: false},
		{name: "cloudflare html", code: http.StatusForbidden, body: "<!doctype html><title>Just a moment...</title>", want: true},
		{name: "cloudflare header", code: http.StatusForbidden, header: http.Header{"Cf-Mitigated": []string{"challenge"}}, body: `{"error":"forbidden"}`, want: true},
		{name: "generic html forbidden", code: http.StatusForbidden, body: "<!doctype html><title>Forbidden</title>", want: false},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := newWebMediaUpstreamErrorWithHeader(test.code, test.header, []byte(test.body), false)
